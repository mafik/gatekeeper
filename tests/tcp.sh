...
systemd-run --service-type=notify --same-dir --unit=gatekeeper-tcp --setenv=LAN=$DEV_A --quiet build/debug_gatekeeper
GATEKEEPER_STATUS=$?
INVOCATION_ID=$(systemctl show --value -p InvocationID gatekeeper-tcp)
...
# Start iperf server
iperf -s -e -B $GATEKEEPER_IP%$DEV_A &
IPERF_PID=$!

# Start iperf client
ip netns exec $NS iperf -c $GATEKEEPER_IP -t 10 -e
