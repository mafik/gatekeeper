...
def hook_recipe(recipe):
  ...
  GLIBC_OBJDIR = GLIBC_ROOT / 'build' / build_variant.name
  PREFIX = build.PREFIX
  ...
  recipe.add_step(
    ...
    desc=f'Setting up Linux headers for the {build_variant.name} build',
    shortcut=f'install linux-headers')
  ...
  recipe.add_step(
    ...
    desc=f'Configuring {GLIBC_STUB}',
    shortcut=f'configure {GLIBC_STUB}')

  recipe.add_step(
    partial(make.Popen, ['make', 'install'], cwd=GLIBC_OBJDIR),
    inputs=[GLIBC_OBJDIR / 'Makefile'],
    outputs=[PREFIX / 'lib' / 'libc.a', PREFIX / 'lib' / 'libc.so'],
    desc=f'Making {GLIBC_STUB}',
    shortcut=f'make {GLIBC_STUB}')

  GCC_OBJDIR = GCC_ROOT / 'build' / build_variant.name

  recipe.add_step(
      partial(make.Popen, ['mkdir', '-p', GCC_OBJDIR]),
      outputs=[GCC_OBJDIR],
      inputs=[GCC_ROOT],
      desc=f'Making build directory for gcc',
      shortcut=f'mkdir gcc')
  ...
  recipe.add_step(
      ...
      desc=f'Configuring gcc',
      shortcut=f'configure gcc')
